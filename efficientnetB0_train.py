...
with strategy.scope():
    ...
    Y = GlobalAvgPool2D()(efficientnetB0_model.output)
    Y = Dense(units=num_classes, activation='softmax', kernel_initializer=GlorotUniform())(Y)
    efficientnetB0_model = Model(inputs=efficientnetB0_model.input, outputs=Y, name='EfficientNetB0')
efficientnetB0_model.summary(line_length=150)
...
best_weights = "./saved_results/Models/EfficientNet_B0/checkpoint"
efficientnetB0_model.load_weights(best_weights)
...
efficientnetB0_model.load_weights(checkpoint_filepath)

# Evaluate the model
loss, accuracy, top_five_accuracy, top_ten_accuracy, top_hundred_accuracy = efficientnetB0_model.evaluate(test_gen)
